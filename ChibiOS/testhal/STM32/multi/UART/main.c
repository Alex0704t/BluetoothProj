...
int main(void) {
   ...
   */
  uartStart(&PORTAB_UART2, &uart_cfg_1);

  /*
   * Blocking API test, send and restart send.
   */
  sz = 3;
  msg = uartSendTimeout(&PORTAB_UART2, &sz, teststr, TIME_INFINITE);
  if (msg != MSG_OK)
  ...
  sz = 16;
  msg = uartSendFullTimeout(&PORTAB_UART2, &sz, teststr, TIME_INFINITE);
  if (msg != MSG_OK)
    chSysHalt("invalid return code");
